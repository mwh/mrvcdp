...
let cooked : Array<{name: string, r: number, g: number, b: number, h: number, s: number, l: number}> = [];
for (let i = 0; i < raw.length; i += 2) {
    ...
    cooked.push({name, r, g, b, h, s, l});
}

export function nameDist(colour: string | {r : number, g: number, b: number} | [number, number, number]) {
    let r, g, b;
    ...
    let [h, s, l] = rgbToHsl(r, g, b);
    if (l >= 0.95) {
        return {name:"white", distance: 1 - l};
    } else if (l <= 0.075) {
        return {name:"black", distance: l};
    }
    if (s <= 0.05) {
        if (l > 0.666)
            return {name:"light grey", distance: 1 - l};
        else if (l < 0.4)
            return {name:"dark grey", distance: l};
        else
            return {name:"grey", distance: Math.abs(l - 0.5) * 2};
    }
    let best = cooked[0];

    let bestDist = 4;
    for (let c of cooked) {
        let dh = Math.abs(c.h - h);
        if (dh > 180) dh = 360 - dh;
        dh /= 180;
        dh *= 2;
        let ds = Math.abs(c.s - s);
        let dl = Math.abs(c.l - l);
        let dist = dh + ds + dl;
        if (dist < bestDist) {
            best = c;
            bestDist = dist;
    ...
    let bestRGB = cooked[0];
    let bestDistRGB = 3 * 255 * 255;
    for (let c of cooked) {
        let ra = 0.5 * (c.r + r);
        let dr = c.r - r;
        let dg = c.g - g;
        ...
        if (ra < 128) {
            ...
            dist = Math.sqrt(3 * dr*dr + 4 * dg*dg + 2 * db*db);
        }
        if (dist < bestDistRGB) {
            bestRGB = c;
            bestDistRGB = dist;
    ...
    return { name: bestRGB.name, distance: bestDistRGB, specimen: '#' + bestRGB.r.toString(16).padStart(2, '0') + bestRGB.g.toString(16).padStart(2, '0') + bestRGB.b.toString(16).padStart(2, '0'), specimenRGB: {...bestRGB} };
}

function colourDist(c1: { r : number, g : number, b : number }, c2: { r : number, g : number, b : number }) {
    let ra = 0.5 * (c1.r + c2.r);
    let dr = c1.r - c2.r;
    let dg = c1.g - c2.g;
    let db = c1.b - c2.b;
    let dist = dr * dr + dg * dg + db * db;
...
export function nextFrom(colour1: string | {r : number, g: number, b: number} | [number, number, number],
    colour2: string | {r : number, g: number, b: number} | [number, number, number]) {
        let r1, g1, b1;
        if (typeof colour1 == 'string') {
            r1 = parseInt(colour1.slice(1, 3), 16);
            g1 = parseInt(colour1.slice(3, 5), 16);
            b1 = parseInt(colour1.slice(5, 7), 16);
        } else if (Array.isArray(colour1)) {
            [r1, g1, b1] = colour1;
        } else {
            ({r:r1, g:g1, b:b1} = colour1);
        }
        let r2, g2, b2;
        if (typeof colour2 == 'string') {
            r2 = parseInt(colour2.slice(1, 3), 16);
            g2 = parseInt(colour2.slice(3, 5), 16);
            b2 = parseInt(colour2.slice(5, 7), 16);
        } else if (Array.isArray(colour2)) {
            [r2, g2, b2] = colour2;
        } else {
            ({r:r2, g:g2, b:b2} = colour2);
        }
        let dist12 = colourDist({r: r1, g: g1, b: b1}, {r: r2, g: g2, b: b2});
        if (dist12 == 0) r1 -= 1;
        let best = cooked[0];
        let bestDist = 3 * 255 * 255;
        for (let c of cooked) {
            let dist1 = colourDist({r: r1, g: g1, b: b1}, c);
            let dist2 = colourDist({r: r2, g: g2, b: b2}, c);
            if (dist2 < bestDist && dist1 > dist2 && dist1 > dist12 && dist2 != 0 && dist1 != 0) {
                best = c;
                bestDist = dist2;
            }
        }
        console.log(best.name, bestDist, colourDist(best, {r: r1, g: g1, b: b1}), dist12);
        return best;
}

export function nameColour(colour: string | {r : number, g: number, b: number} | [number, number, number]) {
    return nameDist(colour).name;
}